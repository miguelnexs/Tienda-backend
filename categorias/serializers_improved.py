...
class CategoriaProductoSerializer(serializers.ModelSerializer):
    """
    Serializer mejorado para categorías de productos
    """
...
class CategoriaProductoUpdateSerializer(serializers.ModelSerializer):
    """
    Serializer específico para actualización de categorías
    """
    imagen_url = serializers.SerializerMethodField()
    cantidad_productos = serializers.ReadOnlyField()
    ...
    def validate_nombre(self, value):
        ...
        # Obtener la instancia actual (siempre existe en actualización)
        instance = self.instance
        
        # Verificar si ya existe una categoría con el mismo nombre
        # Excluir la instancia actual
        queryset = CategoriaProducto.objects.filter(nombre__iexact=value.strip())
        if instance:
            queryset = queryset.exclude(pk=instance.pk)
    ...
    def _process_image(self, categoria, imagen):
        ...
        try:
            ...
            print(f"❌ Error al procesar imagen: {str(e)}")
            # No lanzar excepción para no interrumpir la actualización de la categoría
            # Solo registrar el error
