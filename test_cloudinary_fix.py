#!/usr/bin/env python
"""
Script para probar y corregir la configuración de Cloudinary
"""
...
def test_api_upload():
    ...
    product_data = {
        'nombre': f'Test Cloudinary Fix {unique_id}',
        'sku': f'TEST-FIX-{unique_id}',
        'slug': f'test-cloudinary-fix-{unique_id}',
        'precio': '99.99',
        'descripcion_corta': 'Test de Cloudinary Fix',
        'descripcion_larga': 'Este es un producto de prueba para verificar que Cloudinary funciona correctamente después de las correcciones.',
        'tipo': 'fisico',
        'estado': 'publicado',
        'gestion_stock': True,
        'stock': 10,
    ...
    try:
        # Preparar los datos
        files = {
            'imagen_principal': ('test_fix.jpg', img_byte_arr, 'image/jpeg')
        }
...
def main():
    """Función principal"""
    print("🚀 Test de corrección de Cloudinary")
    print("=" * 50)
    
    # Test 1: Configuración
    test_cloudinary_config()
    
    # Test 2: Storage de Django
    storage_ok = test_django_storage()
    
    # Test 3: Subida directa
    ...
    print("\n" + "=" * 50)
    print("📊 Resumen:")
    print(f"   Storage Django: {'✅ OK' if storage_ok else '❌ Error'}")
    print(f"   Subida directa: {'✅ OK' if direct_ok else '❌ Error'}")
    print(f"   API: {'✅ OK' if api_ok else '❌ Error'}")
    
    if storage_ok and direct_ok and api_ok:
        print("🎉 ¡Cloudinary está funcionando correctamente!")
    else:
