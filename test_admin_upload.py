#!/usr/bin/env python3
"""
Script para probar subida de imágenes desde el admin de Django a Cloudinary
"""
import os
import sys
import django
...
from Backend.cloudinary_storage_fixed import CloudinaryStorageFixed
from django.core.files.base import ContentFile
from django.core.files import File

def test_admin_style_upload():
    """Probar subida como lo haría el admin de Django"""
    ...
    try:
        ...
        django_file = File(img_io, name='test_admin_upload.jpg')
        
        # Usar el storage directamente
        storage = CloudinaryStorageFixed()
...
def test_producto_model_with_fixed_storage():
    """Probar modelo Producto con el storage corregido"""
    print("\n🧪 Probando modelo Producto con storage corregido...")
    ...
    try:
        ...
        producto = Producto(
            nombre="Producto Test Admin Upload",
            descripcion_corta="Producto para probar subida desde admin",
            descripcion_larga="Producto creado para probar la funcionalidad de subida desde el admin de Django",
            precio=250.00,
            costo=125.00,
            stock=8,
            estado='publicado',
            sku=f'TEST-ADMIN-{os.getpid()}'
        )
        ...
        django_file = File(img_io, name=f'test_producto_admin_{os.getpid()}.png')
        
        # Usar el storage corregido para subir la imagen
        storage = CloudinaryStorageFixed()
        saved_name = storage.save(django_file.name, django_file)
...
def test_categoria_model_with_fixed_storage():
    """Probar modelo CategoriaProducto con el storage corregido"""
    print("\n🧪 Probando modelo CategoriaProducto con storage corregido...")
    
    try:
        from categorias.models import CategoriaProducto
        
        # Crear una imagen de prueba
        img = Image.new('RGB', (400, 400), color='cyan')
        img_io = BytesIO()
        img.save(img_io, format='JPEG')
        img_io.seek(0)
        ...
        categoria = CategoriaProducto(
            nombre="Categoría Test Admin Upload",
        ...
        # Crear archivo de imagen como lo haría el admin
        django_file = File(img_io, name=f'test_categoria_admin_{os.getpid()}.jpg')
...
if __name__ == "__main__":
    print("🚀 INICIANDO PRUEBAS DE SUBIDA DESDE ADMIN")
    print("=" * 60)
    ...
    producto_success = test_producto_model_with_fixed_storage()
    
    # Probar modelo CategoriaProducto
    categoria_success = test_categoria_model_with_fixed_storage()
    ...
    if admin_success and producto_success and categoria_success:
        print("\n🎉 ¡Todas las pruebas pasaron! Las imágenes se suben correctamente a Cloudinary desde el admin.")
