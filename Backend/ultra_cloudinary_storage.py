"""
Storage ultra-agresivo para Cloudinary
"""
import os
from django.core.files.storage import Storage
from django.core.files.base import ContentFile

class UltraCloudinaryStorage(Storage):
    """
    Storage ultra-agresivo para Cloudinary
    """
    ...
    def __init__(self):
        ...
        print(f"🔧 UltraCloudinaryStorage inicializado:")
        print(f"  Cloud Name: {self._cloud_name}")
        print(f"  API Key: {self._api_key[:10]}...")
        print("✅ Cloudinary configurado ultra-agresivamente")
    ...
    def _save(self, name, content):
        """Guardar archivo en Cloudinary - ULTRA AGRESIVO"""
        try:
            import cloudinary.uploader
            
            print(f"📤 SUBIENDO A CLOUDINARY ULTRA AGRESIVO: {name}")
            print(f"  Tamaño del archivo: {len(content.read())} bytes")
            content.seek(0)  # Resetear posición del archivo
            
            # Subir a Cloudinary con configuración ultra-agresiva
            ...
            print(f"✅ SUBIDO A CLOUDINARY ULTRA AGRESIVO EXITOSAMENTE:")
            ...
            print(f"❌ ERROR SUBIENDO A CLOUDINARY ULTRA AGRESIVO: {e}")
            import traceback
            traceback.print_exc()
            raise
    ...
    def url(self, name):
        ...
        try:
            ...
            print(f"🔗 URL generada ultra-agresiva: {url}")
            return url
        except Exception as e:
            print(f"❌ Error generando URL ultra-agresiva: {e}")
            return f"/media/{name}"
